### PathFX version created: so_dist_0.85
...
new_hash = pickle.load(open('../rscs/pathfx_mp_0.99_node_to_hashID.pkl','rb'))
new_hash_sum_files = pickle.load(open('../rscs/pathfx_mp_0.85_spec_nbhd_hash.pkl','rb'))
db = pickle.load(open('../rscs/pathfx_mp_dbid2name.pkl','rb')) # Drugbank ID to name
unn = pickle.load(open('../rscs/pathfx_mp_unique_nodes.pkl','rb'))
dtd = pickle.load(open('../rscs/pathfx_mp_dint.pkl','rb'))
...
def do_network(tlist,aname,outdir,dname,doCluster):
	...
	cmd = 'python ../scripts/get_network_associations_so_dist_0.85.py -f %s -a %s -d %s -n %s' % (froot,dname,outdir,len(tlist))
	os.system(cmd)
