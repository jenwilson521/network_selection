### PathFX version created: so_dist_0.82
...
new_hash_sum_files = pickle.load(open('../rscs/pathfx_mp_0.82_spec_nbhd_hash.pkl','rb'))
...
def do_network(tlist,aname,outdir,dname,doCluster):
	...
	cmd = 'python ../scripts/get_network_associations_so_dist_0.82.py -f %s -a %s -d %s -n %s' % (froot,dname,outdir,len(tlist))
	os.system(cmd)
